        ...
        <div class="p-6 bg-white border-b border-gray-200">
          <form action="{{ route('product.update', $product) }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
              ...
              <input type="text" name="name" value="{{ old('name', $product->name) }}" id="name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:ring focus:ring-opacity-50 focus:border-indigo-300 focus:ring-indigo-200 @error('name') border-red-400 @enderror" autofocus />
              @error('name')
            ...
            <button type="submit" class="w-full mt-4 rounded px-4 py-2 bg-blue-500 text-white hover:bg-blue-600">Edit Produk</button>
          </form>
        </div>
      </div>
    </div>
